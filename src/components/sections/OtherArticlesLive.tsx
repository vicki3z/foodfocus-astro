...
const SHOW = 3;
const CATEGORY_LABEL = "What's In";
...
function currentSlugFromPath(): string {
  ...
  const idx = parts.indexOf("whats-in");
...
export function OtherArticlesLive({ title = "Other Articles" }: { title?: string }) {
  ...
  useEffect(() => {
    let active = true;
    fetch(`${baseUrl}whats-in/latest.json`)
